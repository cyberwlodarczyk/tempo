    ...
    for (i = 0; i < MLKEM_K; i++)
        __loop__(
            assigns(i, memory_slice(a, sizeof(mlk_polymat)))
                invariant(i <= MLKEM_K)
                    invariant(forall(x, 0, MLKEM_K, forall(y, 0, MLKEM_K, array_bound(a->vec[x].vec[y].coeffs, 0, MLKEM_N, 0, MLKEM_Q))))
                        decreases(MLKEM_K - i))
        {
                               ...
                               const uint8_t coins[MLKEM_SYMBYTES])
{
    ...
    mlk_zeroize(&skpv_cache, sizeof(mlk_polyvec_mulcache));
    mlk_zeroize(&skpv, sizeof(mlk_polyvec));
    mlk_zeroize(&pkpv, sizeof(mlk_polyvec));
    mlk_zeroize(&e, sizeof(mlk_polyvec));
    mlk_zeroize(&a, sizeof(mlk_polymat));
    mlk_zeroize(coins_with_domain_separator, MLKEM_SYMBYTES + 1);
                   ...
                   const uint8_t coins[MLKEM_SYMBYTES])
{
    ...
    /* Specification: Partially implements
     * @[FIPS203, Section 3.3, Destruction of intermediate values] */
    mlk_zeroize(&sp_cache, sizeof(mlk_polyvec_mulcache));
    mlk_zeroize(&epp, sizeof(mlk_poly));
    mlk_zeroize(&k, sizeof(mlk_poly));
    mlk_zeroize(&v, sizeof(mlk_poly));
    mlk_zeroize(&b, sizeof(mlk_polyvec));
    mlk_zeroize(&ep, sizeof(mlk_polyvec));
    mlk_zeroize(&pkpv, sizeof(mlk_polyvec));
    mlk_zeroize(&sp, sizeof(mlk_polyvec));
    mlk_zeroize(&at, sizeof(mlk_polymat));
    mlk_zeroize(seed, MLKEM_SYMBYTES);
    return ret;
}
                   ...
                   const uint8_t sk[MLKEM_INDCPA_SECRETKEYBYTES])
{
    ...
    /* Specification: Partially implements
     * @[FIPS203, Section 3.3, Destruction of intermediate values] */
    mlk_zeroize(&b_cache, sizeof(mlk_polyvec_mulcache));
    mlk_zeroize(&sb, sizeof(mlk_poly));
    mlk_zeroize(&v, sizeof(mlk_poly));
    mlk_zeroize(&skpv, sizeof(mlk_polyvec));
    mlk_zeroize(&b, sizeof(mlk_polyvec));
    return ret;
